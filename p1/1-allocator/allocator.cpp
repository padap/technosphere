...
// Global allocate memory
{
  if (COMMENTS) printf("\nAllocate size: %d",size);
...
// allocate local block
  ...
  for (int i=0;i<memory_len;i++){
    if (memory_status[i]==0){   // Loop to know is there anywhere free space
      is_block_free=true;
      for (int j=i;j<N+i;j++)
      ...
      if (is_block_free==true){ // Marker free space
        position=i;
        for (int j=i;j<N+i;j++)
          memory_status[j]=1;
        break;
  ...
  if (position==-1){            // There is no free space
    throw AllocError(NoMemory, "No memory");
    return Pointer();
  }
  my_ptr test_ptr;
  int pointer_num;
  for (int i=0;i<max_ptrs;i++){ // new_pointer is a ref to memory block
...
{ 
// free local block
// free memory, free Pointer ref, set memory_status[..]=0
//
  if (p.get()==NULL){
...
{ 
// Set memory len for block equal N
...
{
// Regularize code blocks
// Go to all memory, see can we move block closer to the begining
